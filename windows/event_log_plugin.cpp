...
// static
void EventLogPlugin::RegisterWithRegistrar(flutter::PluginRegistrarWindows *registrar)
{
    auto method_channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
        registrar->messenger(), "event_log", &flutter::StandardMethodCodec::GetInstance());

    auto event_channel = std::make_unique<flutter::EventChannel<flutter::EncodableValue>>(
        registrar->messenger(), "event_log/events", &flutter::StandardMethodCodec::GetInstance());

    auto plugin = std::make_unique<EventLogPlugin>(registrar, std::move(method_channel),
...
EventLogPlugin::EventLogPlugin(
    ...
    : registrar_(registrar), method_channel_(std::move(method_channel)),
      event_channel_(std::move(event_channel)), manager_(std::make_unique<EventLogManager>())
{
...
void EventLogPlugin::SendEvent(const flutter::EncodableValue &event)
{
    ...
    if (event_sink_)
    {
        event_sink_->Success(SanitizeEncodablePayload(event));
    }
}
    ...
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
{
    ...
    try
    {
        if (method_name == "listChannels")
        {
            ...
            result->Success(SanitizeEncodablePayload(flutter::EncodableValue(channels)));
        }
        else if (method_name == "queryEvents")
        {
            ...
            result->Success(SanitizeEncodablePayload(flutter::EncodableValue(events)));
        }
        else if (method_name == "getEventById")
        {
            ...
            if (channel_it != args->end() &&
                std::holds_alternative<std::string>(channel_it->second))
            {
            ...
            result->Success(SanitizeEncodablePayload(event));
        }
        else if (method_name == "subscribeToEvents")
        {
            ...
            auto subscription_id = manager_->SubscribeToEvents(
                *args, [this](flutter::EncodableValue event) { this->SendEvent(event); });
        ...
        else if (method_name == "getChannelInfo")
        {
            ...
            result->Success(SanitizeEncodablePayload(info));
        }
        else
        {
    ...
    {
        result->Error(NormalizeUtf8Bytes(e.code()), NormalizeUtf8Bytes(e.what()),
                      SanitizeEncodablePayload(e.details()));
    }
    catch (const std::exception &e)
    {
        result->Error("EXCEPTION", NormalizeUtf8Bytes(e.what()));
    }
}
