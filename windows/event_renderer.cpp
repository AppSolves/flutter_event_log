...
flutter::EncodableMap EventRenderer::GetSystemProperties(EVT_HANDLE event_handle)
{
    ...
    EvtRender(render_context_, event_handle, EvtRenderEventValues, buffer_size, nullptr,
    ...
    if (!EvtRender(render_context_, event_handle, EvtRenderEventValues, buffer_size, buffer.data(),
                   &buffer_used, &property_count))
    {
        return props;
    }
    ...
    {
        props[flutter::EncodableValue("keywords")] = flutter::EncodableValue(values[7].Int64Val);
    }
...
std::string EventRenderer::RenderEventAsXml(EVT_HANDLE event_handle)
{
    ...
    // First call to get buffer size
    EvtRender(nullptr, event_handle, EvtRenderEventXml, buffer_size, nullptr, &buffer_used,
              &property_count);
    ...
    // Second call to get actual data
    if (!EvtRender(nullptr, event_handle, EvtRenderEventXml, buffer_size, buffer.data(),
                   &buffer_used, &property_count))
    {
        return "";
    }
                                           ...
                                           const std::string &provider_name)
{
    ...
    EVT_HANDLE publisher_metadata =
        EvtOpenPublisherMetadata(nullptr, provider_wide.c_str(), nullptr, 0, 0);
    ...
    DWORD buffer_used = 0;
    EvtFormatMessage(publisher_metadata, event_handle, 0, 0, nullptr, EvtFormatMessageEvent, 0,
                     nullptr, &buffer_used);
    ...
    if (!EvtFormatMessage(publisher_metadata, event_handle, 0, 0, nullptr, EvtFormatMessageEvent,
                          static_cast<DWORD>(buffer.size()), buffer.data(), &buffer_used))
    {
        EvtClose(publisher_metadata);
        return "";
    }
...
std::string EventRenderer::WideToUtf8(const std::wstring &wide)
{
    ...
    std::wstring sanitized = wide;
    if (size_needed == 0)
    {
        sanitized = SanitizeWideUtf16(wide);
        size_needed =
    ...
    if (WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, sanitized.c_str(),
                            static_cast<int>(sanitized.size()), result.data(), size_needed, nullptr,
                            nullptr) == 0)
    {
        return "";
    }
...
std::wstring EventRenderer::Utf8ToWide(const std::string &utf8)
{
    ...
    int size_needed = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8.c_str(),
                                          static_cast<int>(utf8.size()), nullptr, 0);
    if (size_needed == 0)
    {
        size_needed = MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), static_cast<int>(utf8.size()),
                                          nullptr, 0);
    }
    if (size_needed <= 0)
    {
    ...
    if (MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), static_cast<int>(utf8.size()), result.data(),
                            size_needed) == 0)
    {
        return L"";
    }
